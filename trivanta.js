...
// FIXED: Made async and properly returns vendor name
async function getVendorName(user) {
    if (!user) return "N/A";
    ...
    try {
        ...
        const snapshot = await db.collection('vendors').where('email', '==', user.email).get();
        if (!snapshot.empty) {
...
// FIXED: Made async to properly await getVendorName
auth.onAuthStateChanged(async (user) => {
    if (document.getElementById('vendor-portal')) {
        if (user && user.email.includes(VENDOR_DOMAIN) && user.email !== ADMIN_EMAIL) {
            ...
            if (nameDisplay) {
                nameDisplay.innerHTML = '<i class="fas fa-spinner fa-spin"></i>';
                const vendorName = await getVendorName(user);
                nameDisplay.textContent = vendorName;
...
async function addCandidate(event) {
    ...
    const vendorName = await getVendorName(user); // FIXED: Added await
    ...
    try {
        ...
        if (resumeFile) {
            ...
            const storageRef = storage.ref(`resumes/${vendorName}/${Date.now()}-${sanitizedFileName}`);
...
function setupAnalyticsListener() {
    db.collection('candidates').onSnapshot(snapshot => {
        ...
        const totalKpi = document.getElementById('total-candidates-kpi');
        if (totalKpi) {
            totalKpi.textContent = totalCandidates;
        }
...
// FIXED: Removed insecure password prompt, uses secondary Firebase app
async function addNewVendor(event) {
    ...
    try {
        // FIXED: Use secondary Firebase app to avoid signing out admin
        secondaryApp = firebase.initializeApp(firebaseConfig, 'Secondary-' + Date.now());
        const secondaryAuth = secondaryApp.auth();
        
        // Create Firebase Auth user in secondary app
        const userCredential = await secondaryAuth.createUserWithEmailAndPassword(vendorEmail, vendorPassword);
        
        // Store vendor details in Firestore (using primary app's db)
...
function loadVendorList() {
    ...
    db.collection('vendors').orderBy('createdAt', 'desc').onSnapshot(async (snapshot) => {
        ...
        // FIXED: Process vendors sequentially to avoid rate limiting
        for (const doc of snapshot.docs) {
            ...
            try {
                ...
                row.innerHTML = `
                    ...
                    <td>
                        <button class="btn btn-sm btn-danger" onclick="deleteVendor('${doc.id}', '${vendorData.email}', '${vendorData.uid}')">
                            <i class="fas fa-trash"></i>
                        </button>
                    </td>
                `;
...
// FIXED: Added uid parameter and better warning message
async function deleteVendor(vendorDocId, vendorEmail, vendorUid) {
    if (!confirm(`⚠️ WARNING: This will permanently delete this vendor and ALL their candidates!\n\nNote: The vendor's login account will remain active and must be disabled manually in Firebase Console (Authentication section).\n\nContinue with deletion?`)) {
        return;
    }
    ...
    try {
        ...
        showNotification(`Vendor data deleted. To disable login: Go to Firebase Console > Authentication > Find ${vendorEmail} > Click 3 dots > Disable user`, 'warning');
...
async function deleteCandidate(candidateId) {
    ...
    try {
        // FIXED: Also delete resume file if exists
        const candidateDoc = await db.collection('candidates').doc(candidateId).get();
        if (candidateDoc.exists) {
            const data = candidateDoc.data();
            if (data.resumeURL) {
                try {
                    const resumeRef = storage.refFromURL(data.resumeURL);
                    await resumeRef.delete();
                } catch (storageError) {
                    console.warn("Resume file not found or already deleted:", storageError);
                }
            }
        }
...
function showNotification(message, type = 'info') {
    ...
    // Auto remove after 7 seconds (longer for warning messages)
    setTimeout(() => {
        if (notification.parentElement) {
            notification.remove();
        }
    }, type === 'warning' ? 10000 : 7000);
}
